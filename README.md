...
encoded files **without** a Xing header.
Unfortunately, Volvo Sensus will not report correct duration time for variable bitrate encodings (such as ABR and VBR) when the Xing header is removed.
Which is consistent, since this header explicitly provides additional pieces of information for this specific goal.

After more reverse engineering, I was able to determine the exact source of incompatibility was coming from only two bits of information in the MPEG header.
Namely:
1. The "original" bit must be set
2. The "channel mode" bits must be set to "Joint Stereo" (a mode where both stereo channel are encoded as (L+R)/2 and (L-R)/2).
A [https://code.ffmpeg.org/FFmpeg/FFmpeg/issues/24129](ticket) was opened on FFMPEG bug tracker.
