...
When I first tried to listen to my music collection from a USB flash drive, I quickly discovered that the media player was far more restrictive than the official documentation suggested.
While the documentation simply states that MP3 files are supported, it does not mention the numerous practical limitations imposed by the firmware.
...
At the time, several online discussions reported that MP3 files encoded directly with **LAME** worked flawlessly on Volvo Sensus, while some files produced by **FFmpeg** did not.
One discussion on the FFmpeg mailing list from 2014 reports exactly this behavior on a Volvo Sensus 3.0 system, although the underlying cause remained unidentified.

I eventually wrote a collection of Bash scripts combining **FFmpeg** and **LAME** to generate MP3 files accepted by the Sensus system.
Those scripts also split the music library into two partitions of roughly equal size.
...
The project was developed with the assistance of **ChatGPT**, mainly to discuss software architecture, evaluate Python libraries, explore the MP3 format and accelerate some implementation tasks.
The overall design, reverse engineering work, experiments and validation on the actual vehicle were carried out manually.
...
When encoding with **libmp3lame**, FFmpeg writes a **Xing/LAME** header describing the encoding parameters.
This header is normally used to improve duration estimation and seeking accuracy, especially for variable bitrate (VBR) files.
...
* the main difference lies in the way FFmpeg writes the first MPEG header and the Xing/LAME header.

A first workaround was to disable FFmpeg's Xing/Info header with `-write_xing 0` FFMPEG option.

This makes the resulting files compatible with Volvo Sensus, and extensive testing showed that the player can decode CBR, ABR and VBR MP3 streams without a Xing/Info header.

However, removing the Xing/Info header had a drawback: duration reporting and seeking become unreliable for ABR and VBR files, because the player no longer has access to the frame count, byte count and table of contents normally stored in this header.

Further reverse engineering showed that the Xing/LAME metadata itself is not the real cause of the incompatibility.
The decisive difference is located in the MPEG header of the first frame, the frame that carries the Xing/Info metadata.

Volvo Sensus requires at least the following properties in this first MPEG frame header:

1. the original bit must be set;
2. the channel mode field must be set to Joint Stereo.

This means the Xing/Info header can be preserved.
The current implementation lets FFmpeg write its normal Xing/LAME metadata, then patches the first MPEG frame header and rewrites selected LAME tag fields for consistency.


A [ticket](https://code.ffmpeg.org/FFmpeg/FFmpeg/issues/24129) was opened on FFMPEG bug tracker.
...
* **CBR** encoding works correctly without a Xing header;
* **ABR** and **VBR** encodings play correctly without a Xing header;
* bit time reporting and seeking is not fully functional without the Xing header;
* patching few bytes in MPEG header make tracks fully compatible with Volvo Sensus;
